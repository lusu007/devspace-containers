...
curl -s "https://api.github.com/repos/loft-sh/devspace/releases/$DEVSPACE_VERSION" | grep 'browser_download_url.*devspace-linux-'$ARCH_SHORT | cut -d : -f 2,3 | tr -d \" | grep -v '.sha256' | wget -qi -O devspace -
chmod +x devspace
...
curl -s https://api.github.com/repos/loft-sh/loft/releases/latest | grep 'browser_download_url.*loft-linux-'$ARCH_SHORT | cut -d : -f 2,3 | tr -d \" | grep -v '.sha256' | wget -qi -O loft -
chmod +x loft
